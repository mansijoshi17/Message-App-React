                    ...
                    <Mutation mutation={ADD_MESSAGE}>
                        {(addMessage) => {
                            return <input type="text" className="pt-3 pb-3" placeholder="Send Message..." style={{ borderRadius: '0px' }} onChange={onChangeHandler} onKeyUp={(e) => {
                                return e.keyCode === 13 ?  addMessage({ variables: { input: { parentid: parseInt(localStorage.getItem("Id")), type: "sender", name: "Mansi", message: this.state.Message } } }, window.location.reload()) : null
